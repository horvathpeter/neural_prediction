...
class MlpBackend(BaseBackend):
    def __init__(self, inpmulti, hidmulti, outmulti, learning_rate, inp96, hid96, out96, path, buffsize, mean, std, statspath):
        ...
        super(MlpBackend, self).__init__(buffsize)
    ...
    def train(self):
        log2 = self.model_multistep.fit(self.X_train_multistep, self.y_train_multistep, batch_size=1, nb_epoch=2,
                                        validation_split=0.1, verbose=1)
        log3 = self.model_onestep96.fit(self.X_train_onestep96, self.y_train_onestep96, batch_size=1, nb_epoch=2,
